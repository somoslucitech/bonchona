...
interface EstudioClientProps {
  whatsappAdvertising: string;
}

export default function EstudioClient({ whatsappAdvertising }: EstudioClientProps) {
  return (
    <div className="flex flex-col items-center w-full bg-bonchona-navy text-white min-h-screen overflow-x-hidden">
      ...
      <section className="relative w-full min-h-[85dvh] sm:min-h-[90dvh] flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-24 py-20 sm:py-24 z-10">
        <div className="max-w-7xl w-full">
          <motion.div
            ...
            className="inline-flex items-center gap-3 px-5 sm:px-6 py-2 sm:py-2.5 rounded-full glass border-white/10 mb-8 sm:mb-10 shadow-xl"
          >
             <span className="w-2 h-2 bg-bonchona-red rounded-full animate-pulse"></span>
             <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.4em] sm:tracking-[0.5em] text-white/60">The Production House</span>
          </motion.div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center z-10 border-t border-white/5">
         <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
            <span className="text-bonchona-purple-medium font-black tracking-[0.4em] text-[10px] sm:text-xs uppercase mb-6 sm:mb-8 block">Nuestra Visión</span>
            <h2 className="text-4xl sm:text-6xl md:text-7xl font-black italic uppercase tracking-tighter mb-8 leading-[0.9]">
               TU VISIÓN, <br /> NUESTRO <span className="text-bonchona-red">ESPACIO.</span>
            </h2>
            <p className="text-zinc-500 text-base sm:text-lg leading-relaxed mb-8 sm:mb-10 max-w-lg font-medium">
               ...
               {[
                 ...
                 "Staff Especializado"
                  ...
                  <div key={i} className="flex items-center gap-4 group p-4 glass rounded-2xl border-white/5">
                     ...
                     <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors">{item}</span>
                  </div>
               ))}
            </div>
         </div>
         <div className="relative group px-4 sm:px-0">
            ...
            <div className="relative glass rounded-[2.5rem] sm:rounded-[3rem] overflow-hidden border-white/10 p-3 sm:p-4">
              <Image
                 src="https://images.unsplash.com/photo-1590602847861-f357a9332bbc?q=80&w=1974&auto=format&fit=crop"
                 alt="Studio Setup"
                 className="relative rounded-[2rem] sm:rounded-[2.5rem] grayscale group-hover:grayscale-0 transition-all duration-1000 object-cover w-full h-auto"
                 width={800}
                 height={600}
              />
            </div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 z-10 border-t border-white/5">
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left mb-12 sm:mb-16">
           <h3 className="text-bonchona-purple-medium font-black tracking-[0.4em] sm:tracking-[0.5em] text-[10px] sm:text-xs uppercase mb-6 block">Portafolio de Servicios</h3>
           <h2 className="text-4xl sm:text-6xl md:text-7xl font-black italic uppercase tracking-tighter leading-none">SOLUCIONES DE <br /><span className="text-gradient">GRADO BROADCAST.</span></h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {SERVICES.map((service, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -12 }}
              className="group relative p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] glass border-white/5 hover:border-bonchona-red/40 transition-all duration-700 flex flex-col h-full shadow-xl"
            >
              <div className="text-3xl sm:text-4xl mb-6 sm:mb-8 group-hover:scale-110 transition-transform duration-700">{service.icon}</div>
              <h3 className="text-2xl sm:text-3xl font-black uppercase italic mb-4 sm:mb-6 tracking-tighter text-white group-hover:text-bonchona-red transition-colors leading-none">
                {service.title}
              </h3>
              <p className="text-zinc-500 leading-relaxed text-sm sm:text-base mb-8 sm:mb-10 flex-1 font-medium">
                {service.desc}
              </p>

              <ul className="grid grid-cols-1 gap-3 sm:gap-4 mb-8 sm:mb-10">
                 {service.features.map((f, j) => (
                    <li key={j} className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-3 leading-tight">
                       <span className="w-1.5 h-1.5 bg-bonchona-red rounded-full flex-shrink-0"></span> {f}
                    </li>
                 ))}
              </ul>

              <a
                href={`https://wa.me/${whatsappAdvertising}?text=${encodeURIComponent(service.msg)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-4 sm:py-5 bg-bonchona-red text-white font-black rounded-2xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest text-[10px] text-center"
              >
                Contratar Servicio
              </a>
            </motion.div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-24 sm:py-32 text-center z-10 border-t border-white/5">
         <motion.h2
           whileInView={{ opacity: 1, scale: 1 }}
           initial={{ opacity: 0, scale: 0.9 }}
           className="text-4xl sm:text-7xl md:text-9xl font-black uppercase italic tracking-tighter mb-12 sm:mb-16 leading-none"
         >
         ...
         <div className="flex flex-col md:flex-row items-center justify-center gap-8 sm:gap-10">
            <motion.a
               ...
               whileTap={{ scale: 0.95 }}
               href={`https://wa.me/${whatsappAdvertising}?text=Hola%20Bonchona!%20Deseo%20hacer%20una%20consulta%20general%20sobre%20sus%20servicios%20de%20contrataci%C3%B3n.`}
               target="_blank"
               rel="noopener noreferrer"
               className="px-10 sm:px-14 py-5 sm:py-6 bg-bonchona-red text-white font-black rounded-full shadow-[0_20px_40px_rgba(232,75,50,0.3)] uppercase tracking-[0.2em] text-[10px] sm:text-xs w-full sm:w-auto"
            >
               Consultar ahora
            </motion.a>
            <Link href="/" className="text-zinc-500 hover:text-white transition-colors uppercase font-black tracking-[0.4em] text-[9px] sm:text-[10px]">
               Explorar Radio
            </Link>
         </div>
      </section>
