...
export default function FamosoClient({ initialRotativeRates, initialPrograms }: FamosoClientProps) {
  ...
  const getAccent = (id: string) => {
    ...
    if (id === 'concursos-y-variedades') return 'from-bonchona-purple/20';
    if (id === 'te-lo-dije') return 'from-bonchona-rubine/20';
    return 'from-bonchona-navy/40';
  };

  const livePrograms = initialPrograms.map(p => ({
    title: p.title,
    time: p.time,
    price: p.price || 0,
    banner: p.banner,
  ...
  return (
    <div className="flex flex-col items-center w-full bg-bonchona-navy text-white min-h-screen overflow-x-hidden">
      ...
      <section className="relative w-full py-16 sm:py-20 px-4 sm:px-6 text-center max-w-7xl mx-auto z-10">
        <motion.div 
        ...
        >
           ...
           <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.4em] sm:tracking-[0.5em] text-white/60">Soluciones Comerciales</span>
        </motion.div>
        ...
        <motion.p 
          ...
          className="text-zinc-400 text-base sm:text-lg md:text-xl max-w-3xl mx-auto leading-relaxed font-medium mb-10 sm:mb-12 px-4 sm:px-0"
        >
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 z-10 border-t border-white/5">
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-end mb-16 sm:mb-24 gap-10 text-center lg:text-left">
          <div className="flex flex-col items-center lg:items-start max-w-xl">
            <span className="text-bonchona-purple-medium font-black tracking-[0.4em] text-[10px] sm:text-xs uppercase mb-6 block">Tarifario 2026</span>
            <h2 className="text-4xl sm:text-6xl md:text-7xl font-black italic uppercase tracking-tighter leading-none">PUBLICIDAD <br /><span className="text-gradient">ROTATIVA.</span></h2>
            <p className="text-zinc-500 mt-8 sm:mt-10 text-base sm:text-lg font-medium">Spots distribuidos estratégicamente a lo largo de nuestra programación diaria (Lunes a Viernes).</p>
          </div>
          
          <div className="flex flex-wrap justify-center gap-2 glass rounded-[2rem] sm:rounded-full p-2 border-white/10 shadow-xl">
            {initialRotativeRates.map((rate, i) => (
              <button
                key={i}
                onClick={() => setActiveRotative(i)}
                className={`px-4 sm:px-8 py-2.5 sm:py-3 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${activeRotative === i ? 'bg-bonchona-red text-white shadow-xl' : 'text-zinc-500 hover:text-white'}`}
              >
                {rate.freq}
              </button>
            ))}
        ...
        {initialRotativeRates.length > 0 && initialRotativeRates[activeRotative] && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {initialRotativeRates[activeRotative].durations.map((d, i) => (
              <motion.div 
                key={i}
                whileHover={{ y: -10 }}
                className="p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] glass border-white/5 hover:border-bonchona-red/40 transition-all group flex flex-col items-center text-center shadow-lg"
              >
                <span className="text-bonchona-purple-medium text-[9px] sm:text-[10px] font-black uppercase tracking-[0.4em] mb-4 sm:mb-6">Duración</span>
                <h4 className="text-3xl sm:text-4xl font-black italic mb-8 sm:mb-10 text-white leading-none">{d.time}</h4>
                <div className="w-full h-px bg-white/5 mb-8 sm:mb-10"></div>
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 z-10 border-t border-white/5 bg-bonchona-navy/40 relative">
        <div className="absolute inset-0 bg-mesh-brand opacity-10 pointer-events-none"></div>
        
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left mb-16 sm:mb-24 relative z-10">
           <span className="text-bonchona-red font-black tracking-[0.4em] text-[10px] sm:text-xs uppercase mb-6 block">Asociación Premium</span>
           <h2 className="text-4xl sm:text-6xl md:text-7xl font-black italic uppercase tracking-tighter leading-none mb-8 sm:mb-10">
             PRESENCIA <br /> <span className="text-gradient">EN VIVO.</span>
           </h2>
           <p className="text-zinc-500 max-w-xl text-base sm:text-lg font-medium leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 relative z-10">
          {livePrograms.filter(prog => prog.price > 0).map((prog, i) => (
            <motion.div 
              key={i} 
              whileHover={{ y: -8 }}
              className="group relative h-[420px] sm:h-[480px] rounded-[2.5rem] sm:rounded-[3rem] overflow-hidden border border-white/5 shadow-xl transition-all duration-700"
            >
              <Image 
                src={prog.banner || "/logos-bonchona/92.png"} 
                alt={prog.title} 
                fill 
              ...
              <div className="absolute inset-0 p-8 sm:p-12 flex flex-col justify-between">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="text-bonchona-red font-black text-[9px] sm:text-[10px] tracking-[0.3em] sm:tracking-[0.4em] uppercase truncate">{prog.time}</span>
                    <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter leading-none truncate">{prog.title}</h3>
                  </div>
                  <div className="w-12 h-12 sm:w-16 sm:h-16 flex-shrink-0 glass rounded-full flex items-center justify-center border-white/20">
                    <Image src="/logos-bonchona/92.png" alt="Logo" width={32} height={32} className="w-6 sm:w-8 h-auto opacity-30 group-hover:opacity-100 transition-opacity object-contain" />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-6 sm:gap-8">
                  <div className="grid grid-cols-3 gap-2 sm:gap-3">
                    {["Apertura", "Cierre", "2 Menciones"].map((inc, idx) => (
                      <div key={idx} className="flex flex-col items-center gap-2 p-2 sm:p-3 glass rounded-xl border-white/5">
                        <div className="w-1 h-1 rounded-full bg-bonchona-red"></div>
                        <span className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest text-zinc-400 group-hover:text-white transition-colors">{inc}</span>
                      </div>
                    ))}
                  </div>

                  <div className="flex items-center justify-between gap-4 pt-6 sm:pt-8 border-t border-white/10">
        ...
        <div className="mt-16 sm:mt-24 text-center relative z-10">
          <p className="text-zinc-600 font-black uppercase tracking-[0.4em] sm:tracking-[0.6em] text-[8px] sm:text-[9px] italic underline underline-offset-[10px] decoration-bonchona-red/30">
      ...
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 z-10 border-t border-white/5 overflow-visible">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
            <span className="text-bonchona-purple-medium font-black text-[10px] sm:text-xs tracking-[0.4em] uppercase mb-8 sm:mb-10 block">Audio Branding</span>
            <h2 className="text-4xl sm:text-6xl md:text-7xl font-black mb-8 sm:mb-10 tracking-tight uppercase italic text-white leading-[0.9]">
               ¿SIN <br /> <span className="text-gradient">SPOT?</span>
            </h2>
            <p className="text-zinc-500 mb-8 text-lg sm:text-xl leading-relaxed font-medium italic">
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6">
            {[
              ...
              <motion.div 
                key={i} 
                whileHover={{ x: 8 }}
                className="p-8 sm:p-10 glass border-white/5 rounded-[2.5rem] sm:rounded-[3rem] group hover:border-white/20 transition-all relative overflow-hidden text-center sm:text-left"
              >
                <div className={`absolute top-0 right-0 p-6 sm:p-8 text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-${prod.c} opacity-40`}>{prod.s}</div>
                <h4 className="text-2xl md:text-3xl font-black mb-4 text-white italic uppercase leading-none">{prod.t}</h4>
                <p className="text-zinc-400 text-sm sm:text-base md:text-lg font-medium leading-relaxed italic max-w-sm mx-auto sm:mx-0">{prod.d}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-10 sm:gap-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center w-full gap-8 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center gap-6 sm:gap-8">
              ...
              <Link href="/" className="text-[9px] sm:text-[10px] font-bold text-bonchona-red hover:text-white transition-colors uppercase tracking-widest">Escuchar</Link>
            </div>
          </div>
        </div>
      </footer>
