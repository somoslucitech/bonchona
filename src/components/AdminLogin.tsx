import Image from 'next/image';

const ERROR_MESSAGES: Record<string, string> = {
  not_invited: "Ese correo no tiene una invitación activa. Solicita acceso a un administrador.",
  revoked: "El acceso de esta cuenta fue revocado.",
  invalid_state: "La verificación de seguridad expiró. Intenta de nuevo.",
  oauth_failed: "No se pudo completar el inicio de sesión con Google. Intenta de nuevo.",
  oauth_denied: "Cancelaste el inicio de sesión con Google.",
  email_not_verified: "Tu cuenta de Google no tiene el correo verificado.",
  oauth_not_configured: "El inicio de sesión con Google no está configurado en el servidor.",
};

export default function AdminLogin({ error }: { error?: string }) {
  const errorMessage = error ? (ERROR_MESSAGES[error] ?? "Ocurrió un error al iniciar sesión.") : null;

  return (
    <div className="min-h-screen bg-bonchona-navy text-white flex items-center justify-center px-4 relative overflow-hidden">
      {/* Mesh Background */}
      <div className="fixed inset-0 bg-mesh-brand pointer-events-none opacity-20 z-0"></div>

      <div className="relative z-10 w-full max-w-md">

        {/* Branding Logo */}
        <div className="flex flex-col items-center mb-8 text-center">
          <Image
            src="/logos-bonchona/92.png"
            alt="Bonchona Logo"
            width={90}
            height={90}
            className="object-contain drop-shadow-[0_10px_20px_rgba(232,75,50,0.3)] mb-4"
          />
          <h1 className="text-3xl font-black italic uppercase tracking-tighter">
            PANEL <span className="text-gradient">CONTROL.</span>
          </h1>
          <p className="text-zinc-500 text-[10px] tracking-widest uppercase mt-2 font-black">
            Radio Bonchona 107.1 FM
          </p>
        </div>

        {/* Login Card */}
        <div className="glass rounded-[2rem] p-8 sm:p-10 border-white/10 shadow-2xl flex flex-col items-center gap-6">
          <h2 className="text-sm font-black uppercase tracking-widest text-zinc-400 text-center">
            Acceso exclusivo para el equipo autorizado
          </h2>

          {errorMessage && (
            <div className="w-full p-4 rounded-xl bg-bonchona-red/10 border border-bonchona-red/20 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-bonchona-red">{errorMessage}</p>
            </div>
          )}

          <a
            href="/api/auth/google"
            className="w-full py-4 bg-white text-black font-black rounded-xl hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest text-[11px] shadow-lg flex items-center justify-center gap-3"
          >
            <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
              <path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z" />
              <path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z" />
              <path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z" />
              <path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 0 1-4.087 5.571l.003-.002 6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z" />
            </svg>
            Continuar con Google
          </a>
        </div>

        <div className="text-center mt-8">
          <p className="text-[8px] font-black uppercase tracking-widest text-zinc-600">
            Acceso restringido para personal autorizado
          </p>
        </div>

      </div>
    </div>
  );
}
